...
public class BackgroundStoryQuestionsIndex(Gw2Client sut)
{
    ...
    {
        (IImmutableValueSet<int> actual, MessageContext context) = await sut.Hero.StoryJournal.GetBackgroundStoryQuestionsIndex(TestContext.Current!.Execution.CancellationToken);
