...
[ServiceDataSource]
public class SkillsByPage(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_page()
    {
        const int pageSize = 3;
        (IImmutableValueSet<Skill> actual, MessageContext context) = await sut.Hero.Builds.GetSkillsByPage(0, pageSize, cancellationToken: TestContext.Current!.Execution.CancellationToken);
