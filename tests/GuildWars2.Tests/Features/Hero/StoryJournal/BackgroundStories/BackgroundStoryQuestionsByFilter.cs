...
[ServiceDataSource]
public class BackgroundStoryQuestionsByFilter(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_id()
    {
        HashSet<int> ids = [7, 10, 11];
        (IImmutableValueSet<BackgroundStoryQuestion> actual, MessageContext context) = await sut.Hero.StoryJournal.GetBackgroundStoryQuestionsByIds(ids, cancellationToken: TestContext.Current!.Execution.CancellationToken);
