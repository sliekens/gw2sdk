...
public sealed class ItemPricesByIdsRequest : IHttpRequest<IReplicaSet<ItemPrice>>
{
    ...
    public async Task<IReplicaSet<ItemPrice>> SendAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        ...
        var value = json.RootElement.GetSet(entry => ItemPriceReader.Read(entry, MissingMemberBehavior));
        return new ReplicaSet<ItemPrice>(response.Headers.Date.GetValueOrDefault(),
            value,
