...
public class MistChampionByFilter(Gw2Client sut)
{
    ...
    public async Task Can_be_filtered_by_id()
    {
        using (Assert.Multiple())
        {
            ...
            (IImmutableValueSet<MistChampion> actual, MessageContext context) = await sut.Pvp.GetMistChampionsByIds(ids, cancellationToken: TestContext.Current!.Execution.CancellationToken);
