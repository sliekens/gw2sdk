...
{
    public static GuildDecorationRecipe GetGuildDecorationRecipe(this in JsonElement json)
    {
        ...
        return new GuildDecorationRecipe
        {
            ...
            GuildIngredients =
                guildIngredients.Map(static (in values) =>
                    values.GetList(static (in value) => value.GetGuildIngredient())
                )
                ?? [],
            OutputUpgradeId = outputUpgradeId.Map(static (in value) => value.GetInt32()),
