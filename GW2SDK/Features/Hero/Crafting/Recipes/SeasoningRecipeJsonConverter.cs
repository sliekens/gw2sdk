...
internal sealed class SeasoningRecipeJsonConverter : JsonConverter<SeasoningRecipe>
{
    ...
    public override SeasoningRecipe Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, SeasoningRecipe value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static SeasoningRecipe Read(JsonElement json)
    {
