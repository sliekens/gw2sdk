...
public class Hearts
{
    ...
    public async Task Can_be_listed(int continentId, int floorId, int regionId, int mapId)
    {
        ...
        (HashSet<Heart> actual, MessageContext context) = await sut.Exploration.GetHearts(continentId, floorId, regionId, mapId, cancellationToken: TestContext.Current!.CancellationToken);
