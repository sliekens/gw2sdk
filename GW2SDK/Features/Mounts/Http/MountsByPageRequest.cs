...
public sealed class MountsByPageRequest : IHttpRequest<IReplicaPage<Mount>>
{
    ...
    public async Task<IReplicaPage<Mount>> SendAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        ...
        var value = json.RootElement.GetSet(entry => MountReader.Read(entry, MissingMemberBehavior));
        return new ReplicaPage<Mount>(response.Headers.Date.GetValueOrDefault(),
            value,
