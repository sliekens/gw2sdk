...
internal sealed class ShortbowRecipeJsonConverter : JsonConverter<ShortbowRecipe>
{
    ...
    public override ShortbowRecipe Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, ShortbowRecipe value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static ShortbowRecipe Read(JsonElement json)
    {
