...
[ServiceDataSource]
public class JadeBotSkinsByPage(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_page()
    {
        const int pageSize = 3;
        (IImmutableValueSet<JadeBotSkin> actual, MessageContext context) = await sut.Hero.Equipment.JadeBots.GetJadeBotSkinsByPage(0, pageSize, cancellationToken: TestContext.Current!.Execution.CancellationToken);
