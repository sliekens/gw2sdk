...
[ServiceDataSource]
public class MasteryTracksByFilter(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_id()
    {
        HashSet<int> ids = [1, 2, 3];
        (IImmutableValueSet<MasteryTrack> actual, MessageContext context) = await sut.Hero.Masteries.GetMasteryTracksByIds(ids, cancellationToken: TestContext.Current!.Execution.CancellationToken);
