...
internal static class DyeSlotInfoJson
{
    public static DyeSlotInfo GetDyeSlotInfo(this in JsonElement json)
    {
        ...
        foreach (JsonProperty member in json.EnumerateObject())
        {
            ...
            else if (member.NameEquals("overrides"))
            {
                foreach (JsonProperty @override in member.Value.EnumerateObject())
                {
                    ...
                    {
                        ThrowHelper.ThrowUnexpectedMember(@override.Name);
                    }
                }
            }
            else if (JsonOptions.MissingMemberBehavior == MissingMemberBehavior.Error)
        ...
        static IImmutableValueList<DyeSlot?> GetDyeSlots(in JsonElement values)
        {
            ...
            return values.GetList(static (in value) =>
