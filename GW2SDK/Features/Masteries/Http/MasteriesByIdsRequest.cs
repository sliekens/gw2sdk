...
public sealed class MasteriesByIdsRequest : IHttpRequest<IReplicaSet<Mastery>>
{
    ...
    public async Task<IReplicaSet<Mastery>> SendAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        QueryBuilder search = new();
        search.Add("ids", MasteryIds);
        ...
        var value = json.RootElement.GetSet(entry => MasteryReader.Read(entry, MissingMemberBehavior));
        return new ReplicaSet<Mastery>(response.Headers.Date.GetValueOrDefault(),
            value,
