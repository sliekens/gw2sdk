...
[ServiceDataSource]
public class StoriesByPage(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_page()
    {
        const int pageSize = 3;
        (IImmutableValueSet<Story> actual, MessageContext context) = await sut.Hero.StoryJournal.GetStoriesByPage(0, pageSize, cancellationToken: TestContext.Current!.Execution.CancellationToken);
