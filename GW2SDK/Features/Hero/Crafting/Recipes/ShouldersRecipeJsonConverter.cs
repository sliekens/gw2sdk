...
internal sealed class ShouldersRecipeJsonConverter : JsonConverter<ShouldersRecipe>
{
    ...
    public override ShouldersRecipe Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, ShouldersRecipe value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static ShouldersRecipe Read(JsonElement json)
    {
