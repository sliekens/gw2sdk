...
internal sealed class IngredientCookingRecipeJsonConverter : JsonConverter<IngredientCookingRecipe>
{
    ...
    public override IngredientCookingRecipe Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, IngredientCookingRecipe value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static IngredientCookingRecipe Read(JsonElement json)
    {
