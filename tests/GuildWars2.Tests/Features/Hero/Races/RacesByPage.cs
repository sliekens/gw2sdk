...
[ServiceDataSource]
public class RacesByPage(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_page()
    {
        const int pageSize = 3;
        (IImmutableValueSet<Race> actual, MessageContext context) = await sut.Hero.Races.GetRacesByPage(0, pageSize, cancellationToken: TestContext.Current!.Execution.CancellationToken);
