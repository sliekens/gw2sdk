...
internal sealed class LongbowRecipeJsonConverter : JsonConverter<LongbowRecipe>
{
    ...
    public override LongbowRecipe Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, LongbowRecipe value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static LongbowRecipe Read(JsonElement json)
    {
