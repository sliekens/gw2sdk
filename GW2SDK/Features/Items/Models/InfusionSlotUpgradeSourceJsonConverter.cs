...
internal sealed class InfusionSlotUpgradeSourceJsonConverter : JsonConverter<InfusionSlotUpgradeSource>
{
    public override InfusionSlotUpgradeSource? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using JsonDocument json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, InfusionSlotUpgradeSource value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }
