...
internal static class LaddersJson
{
    public static Ladders GetLadders(this in JsonElement json)
    {
        ...
        return new Ladders
        {
            None = none.Map(static (in value) => value.GetResults()),
            Unranked = unranked.Map(static (in value) => value.GetResults()),
            Ranked = ranked.Map(static (in value) => value.GetResults()),
            Ranked2v2 = ranked2v2.Map(static (in value) => value.GetResults()),
            Ranked3v3 = ranked3v3.Map(static (in value) => value.GetResults()),
            PushRanked = rankedPush.Map(static (in value) => value.GetResults()),
            SoloArenaRated = soloArenaRated.Map(static (in value) => value.GetResults()),
            TeamArenaRated = teamArenaRated.Map(static (in value) => value.GetResults())
        };
    }
}
