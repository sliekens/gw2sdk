...
public sealed class OrderBooksByIdsRequest : IHttpRequest<IReplicaSet<OrderBook>>
{
    ...
    public async Task<IReplicaSet<OrderBook>> SendAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        ...
        var value = json.RootElement.GetSet(entry => OrderBookReader.Read(entry, MissingMemberBehavior));
        return new ReplicaSet<OrderBook>(response.Headers.Date.GetValueOrDefault(),
            value,
