...
{
    public static EliteSkill GetEliteSkill(this in JsonElement json)
    {
...
#pragma warning restore CS0618
            ...
            TransformSkillIds =
                transformSkills.Map(static (in values) =>
            ...
            Categories =
                categories.Map(static (in values) =>
                    values.GetList(static (in value) => value.GetEnum<SkillCategoryName>())
                )
                ?? [],
            SubskillIds =
                subskills.Map(static (in values) => values.GetList(static (in value) => value.GetSubskill())),
            BundleSkillIds =
                bundleSkills.Map(static (in values) => values.GetList(static (in value) => value.GetInt32())),
            Attunement = attunement.Map(static (in value) => value.GetEnum<Attunement>()),
            Cost = cost.Map(static (in value) => value.GetInt32()),
            ToolbeltSkillId = toolbeltSkill.Map(static (in value) => value.GetInt32())
        };
    }
}
