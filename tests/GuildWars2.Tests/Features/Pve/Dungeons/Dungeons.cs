...
public class Dungeons(Gw2Client sut)
{
    ...
    {
        (IImmutableValueSet<Dungeon> actual, MessageContext context) = await sut.Pve.Dungeons.GetDungeons(cancellationToken: TestContext.Current!.Execution.CancellationToken);
