...
internal sealed class ContractNpcJsonConverter : JsonConverter<ContractNpc>
{
    ...
    public override ContractNpc Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, ContractNpc value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static ContractNpc Read(JsonElement json)
    {
