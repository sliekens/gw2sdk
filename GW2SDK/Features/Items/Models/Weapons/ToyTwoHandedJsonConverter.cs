...
internal sealed class ToyTwoHandedJsonConverter : JsonConverter<ToyTwoHanded>
{
    public const string DiscriminatorValue = "toy_two_handed";

    public override bool CanConvert(Type typeToConvert)
    {
        return typeof(ToyTwoHanded).IsAssignableFrom(typeToConvert);
    }

    public override ToyTwoHanded Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, ToyTwoHanded value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static ToyTwoHanded Read(JsonElement json)
    {
        if (!json.GetProperty(ItemJsonConverter.DiscriminatorName).ValueEquals(WeaponJsonConverter.DiscriminatorValue))
        {
        ...
        {
            ThrowHelper.ThrowInvalidDiscriminator(json.GetProperty(WeaponJsonConverter.DiscriminatorName).GetString());
        }
    ...
    public static void Write(Utf8JsonWriter writer, ToyTwoHanded value)
    {
        writer.WriteStartObject();
        writer.WriteString(ItemJsonConverter.DiscriminatorName, WeaponJsonConverter.DiscriminatorValue);
