...
public class ContinentsByFilter(Gw2Client sut)
{
    ...
    {
        HashSet<int> ids = [1, 2];
        (IImmutableValueSet<Continent> actual, MessageContext context) = await sut.Exploration.GetContinentsByIds(ids, cancellationToken: TestContext.Current!.Execution.CancellationToken);
