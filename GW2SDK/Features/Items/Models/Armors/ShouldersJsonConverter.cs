...
internal sealed class ShouldersJsonConverter : JsonConverter<Shoulders>
{
    ...
    public override Shoulders Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, Shoulders value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static Shoulders Read(JsonElement json)
    {
