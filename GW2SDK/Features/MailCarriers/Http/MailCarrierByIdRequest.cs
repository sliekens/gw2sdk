...
public sealed class MailCarrierByIdRequest : IHttpRequest<IReplica<MailCarrier>>
{
    ...
    public async Task<IReplica<MailCarrier>> SendAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        ...
        var value = MailCarrierReader.Read(json.RootElement, MissingMemberBehavior);
        return new Replica<MailCarrier>(response.Headers.Date.GetValueOrDefault(),
            value,
