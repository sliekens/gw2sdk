...
internal sealed class TorchRecipeJsonConverter : JsonConverter<TorchRecipe>
{
    ...
    public override TorchRecipe Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, TorchRecipe value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static TorchRecipe Read(JsonElement json)
    {
