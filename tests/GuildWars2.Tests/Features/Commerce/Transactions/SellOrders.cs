...
public class SellOrders(Gw2Client sut)
{
    ...
    public async Task Current_asks_can_be_filtered_by_page()
    {
        ...
        (IImmutableValueSet<Order> offers, MessageContext context) = await sut.Commerce.GetSellOrders(0, 200, accessToken.Key, cancellationToken: TestContext.Current!.Execution.CancellationToken);
