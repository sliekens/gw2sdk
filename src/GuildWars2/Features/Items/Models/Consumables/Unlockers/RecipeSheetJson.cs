...
{
    public static RecipeSheet GetRecipeSheet(this in JsonElement json)
    {
        ...
        foreach (JsonProperty member in json.EnumerateObject())
        {
            ...
            else if (member.NameEquals("details"))
            {
                foreach (JsonProperty detail in member.Value.EnumerateObject())
                {
                    ...
                    else if (extraRecipeIds.Match(detail))
                    {
        ...
        return new RecipeSheet
        {
            ...
            RecipeId = recipeId.Map(static (in value) => value.GetInt32()),
            ExtraRecipeIds =
                extraRecipeIds.Map(static (in values) => values.GetList(static (in value) => value.GetInt32())
                )
                ?? []
        };
    }
