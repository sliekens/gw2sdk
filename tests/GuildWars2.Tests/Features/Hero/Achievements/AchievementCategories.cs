...
public class AchievementCategories(Gw2Client sut)
{
    ...
    {
        (IImmutableValueSet<AchievementCategory> actual, MessageContext context) = await sut.Hero.Achievements.GetAchievementCategories(cancellationToken: TestContext.Current!.Execution.CancellationToken);
