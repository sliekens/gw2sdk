...
[ServiceDataSource]
public class StorylinesByPage(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_page()
    {
        const int pageSize = 3;
        (IImmutableValueSet<Storyline> actual, MessageContext context) = await sut.Hero.StoryJournal.GetStorylinesByPage(0, pageSize, cancellationToken: TestContext.Current!.Execution.CancellationToken);
