...
internal sealed class GliderSkinUnlockerJsonConverter : JsonConverter<GliderSkinUnlocker>
{
    ...
    public override GliderSkinUnlocker Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, GliderSkinUnlocker value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static GliderSkinUnlocker Read(JsonElement json)
    {
