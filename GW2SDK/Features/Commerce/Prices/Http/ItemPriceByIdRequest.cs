...
public sealed class ItemPriceByIdRequest : IHttpRequest<IReplica<ItemPrice>>
{
    ...
    public async Task<IReplica<ItemPrice>> SendAsync(HttpClient httpClient, CancellationToken cancellationToken)
    {
        ...
        var value = ItemPriceReader.Read(json.RootElement, MissingMemberBehavior);
        return new Replica<ItemPrice>(response.Headers.Date.GetValueOrDefault(),
            value,
