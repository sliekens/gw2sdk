...
internal sealed class StorageExpanderJsonConverter : JsonConverter<StorageExpander>
{
    ...
    public override StorageExpander Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var json = JsonDocument.ParseValue(ref reader);
        return Read(json.RootElement);
    }

    public override void Write(Utf8JsonWriter writer, StorageExpander value, JsonSerializerOptions options)
    {
        Write(writer, value);
    }

    public static StorageExpander Read(JsonElement json)
    {
