...
public class EmblemForegrounds(Gw2Client sut)
{
    ...
    {
        (IImmutableValueSet<EmblemForeground> actual, MessageContext context) = await sut.Guilds.GetEmblemForegrounds(cancellationToken: TestContext.Current!.Execution.CancellationToken);
