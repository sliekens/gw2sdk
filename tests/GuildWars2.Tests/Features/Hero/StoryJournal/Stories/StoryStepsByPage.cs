...
[ServiceDataSource]
public class StoryStepsByPage(Gw2Client sut)
{
    [Test]
    public async Task Can_be_filtered_by_page()
    {
        const int pageSize = 3;
        (IImmutableValueSet<StoryStep> actual, MessageContext context) = await sut.Hero.StoryJournal.GetStoryStepsByPage(0, pageSize, cancellationToken: TestContext.Current!.Execution.CancellationToken);
