...
public sealed class MaterialCategoriesRequest : IHttpRequest<IReplicaSet<MaterialCategory>>
{
    ...
    )
    {
        ...
        var value = json.RootElement.GetSet(entry => MaterialCategoryReader.Read(entry, MissingMemberBehavior));
        return new ReplicaSet<MaterialCategory>(response.Headers.Date.GetValueOrDefault(),
            value,
